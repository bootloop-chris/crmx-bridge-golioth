...
void ssd1306_display_text_box1(SSD1306_t *dev, int page, int seg, char *text,
  ...
  for (int _text = box_width; _text < text_len; _text++) {
    ...
    for (int _bit = 0; _bit < 8; _bit++) {
      ...
      ssd1306_display_image(dev, page, seg, &dev->_page[page]._segs[seg],
                            text_box_pixel);
      vTaskDelay(delay);
    }
  }
...
void ssd1306_display_text_box2(SSD1306_t *dev, int page, int seg, char *text,
                               int box_width, int text_len, bool invert,
                               int delay) {
  if (page >= dev->_pages)
    return;
...
void ssd1306_scroll_text(SSD1306_t *dev, char *text, int text_len,
  ...
  while (1) {
    ...
    i2c_display_image(dev, dstIndex, 0, dev->_page[dstIndex]._segs,
...
void _ssd1306_bitmaps(SSD1306_t *dev, int xpos, int ypos, uint8_t *bitmap,
  ...
  ESP_LOGD(TAG, "_width=%d ypos=%d page=%d dstBits=%d", _width, ypos, page,
           dstBits);
  int offset = 0;
  for (int _height = 0; _height < height; _height++) {
    for (int index = 0; index < _width; index++) {
      for (int srcBits = 7; srcBits >= 0; srcBits--) {
        ...
        ESP_LOGD(TAG, "index=%d offset=%d wk1=0x%x page=%d _seg=%d, wk2=%02x",
                 index, offset, wk1, page, _seg, wk2);
...
void ssd1306_rotate_image(uint8_t *image, bool flip) {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    for (int j = 0; j < 8; j++) {
      ...
      ESP_LOGD(TAG, "image[%d]=0x%x _smask=0x%x _wk=0x%x", j, image[j], _smask,
               _wk);
      if (_wk != 0) {
...
void ssd1306_display_rotate_text(SSD1306_t *dev, int seg, char *text,
